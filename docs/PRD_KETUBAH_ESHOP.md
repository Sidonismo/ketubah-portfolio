...
- Dokumentována správná lokalizace dat v Payload CMS (create + update pattern)
- Dokumentována konfigurace Payload Admin panelu (serverFunction, importMap)
- Aktualizován stav implementace
...
## 20. Implementation Notes (Pro budoucí vývojáře)

> **⚠️ Důležité:** Tato sekce obsahuje praktické zkušenosti z implementace, které nejsou v oficiální dokumentaci.

### 20.1 Payload CMS 3.x - Lokalizace dat

**PRD specifikuje** lokalizovaná pole takto:
```typescript
{
  name: 'name',
  type: 'text',
...
**Skutečná implementace při seedování/vytváření dat:**

```typescript
// ❌ NEFUNGUJE - Payload neakceptuje objekt pro lokalizované pole
await payload.create({
  collection: 'categories',
  data: { name: { cs: 'Tradiční', en: 'Traditional', he: 'מסורתי' } },
});

// ✅ SPRÁVNĚ - Create v default locale, pak update pro každý další
const created = await payload.create({
  collection: 'categories',
  data: { name: 'Tradiční' },  // default locale (cs)
});
...
### 20.2 Payload Admin Panel Setup
...
**Kritická konfigurace `layout.tsx`:**

```typescript
import { handleServerFunctions, RootLayout } from '@payloadcms/next/layouts';
import { ServerFunctionClient } from 'payload';
import { importMap } from './admin/importMap';
import '@payloadcms/next/css';  // ← Nutný CSS import!

// Tato funkce MUSÍ být async s 'use server'
const serverFunction: ServerFunctionClient = async function (args) {
  'use server';
  return handleServerFunctions({ ...args, config, importMap });
};

// RootLayout MUSÍ mít serverFunction prop
<RootLayout importMap={importMap} config={config} serverFunction={serverFunction}>
```

Bez `serverFunction` prop dostanete:
```
Error: ServerFunctionsProvider requires a serverFunction prop
```

### 20.3 Array fields s lokalizovanými sub-fields

**PRD definuje images pole:**
```typescript
{
  name: 'images',
  type: 'array',
...
**Při update locale musíte předat celé pole znovu:**

```typescript
// Při update EN locale - celé images pole se musí předat znovu
await payload.update({
  collection: 'products',
  id: product.id,
  data: {
    name: 'Eternal Love',
    images: [{ image: imageId, alt: 'English alt text', isMain: true }], // ← Celé pole!
  },
  locale: 'en',
});
```
...
**PRD specifikuje `.env.local`**, ale seed scripty vyžadují `.env`:
...
Nebo v seed scriptu:
```typescript
import 'dotenv/config';  // První řádek souboru!
```

### 20.5 Media Upload v seed scriptech

**Payload vyžaduje specifický formát pro file upload:**
...
**Pozor:** `sharp` package je vyžadován pro image resizing:
```bash
npm install sharp
```

### 20.6 Stav implementace (aktualizováno 2025-12-10)

| Oblast | PRD specifikace | Skutečný stav |
|--------|-----------------|---------------|
| Kolekce Payload | 9 kolekcí | ✅ 9/9 implementováno |
